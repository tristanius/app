		...
		<table class="table table-stripped">
			...
			<tbody>
				<tr ng-repeat=" it in usuarios ">
					...
					<td> {{ it.username }} </td>
					<td> {{ it.correo }} </td>
					<td> {{ getRolName(it)+it.nombre_rol }}</td>
					<td> {{ it.base_idbase }} </td>
					<td> {{ it.estado }}</td>
					<td> 
						<a class="btn btn-default" style="padding:3px" href="<?= site_url('administrator/resetPass/') ?>/{{it.idusuario}}">R. Pass</a>
						<a class="btn btn-default" style="padding:3px" href="<?= site_url('administrator/invalidarAcceso/') ?>/{{it.idusuario}}">Invalidar</a>
					</td>
				</tr>
			</tbody>
		</table>
	...
	<script type="text/javascript">
		...
		app.controller("privilegio",function($scope, $http){
			...
			$scope.getRolName = function(user){
				angular.forEach($scope.roles, function(val, key){
					if( val.idrol == user.rol_idrol ){
						user.nombre_rol = val.nombre_rol;
					}
				});
			}
		});
